...
var topCmd = &cobra.Command{
	...
	Long: `Top displays CPU and memory usage of all running Firecracker microVMs.

Default mode prints a single snapshot (agent-friendly).
Use --watch for a live refreshing display (human-friendly).
Use --json for machine-readable output.

Examples:
  umut top            # Single snapshot
  umut top --json     # JSON output for agents
  umut top --watch    # Live refreshing display`,
	Args: cobra.NoArgs,
	RunE: runTop,
}
...
func runTopWatch(store *state.Store, collector *metrics.Collector) error {
	...
	for {
		...
		fmt.Printf("  umut top \033[1m%s\033[0m  %d VM(s) running\n",
